...
def build_daily_tables(df):
    ...
    # in_out 기준
    pivot_inout = df.pivot_table(index='date', columns='in_out',
                                 aggfunc='size', fill_value=0).reset_index()
    for c in ['내항','외항']:
        if c not in pivot_inout.columns: pivot_inout[c] = 0
    pivot_inout['총계'] = pivot_inout['내항'] + pivot_inout['외항']
    show_inout = pivot_inout.copy()
    for c in ['내항','외항','총계']:
        show_inout[c] = show_inout[c].map(lambda x: f"{int(x):,}")

    # entry_exit 기준
    pivot_entryexit = df.pivot_table(index='date', columns='entry_exit',
                                     aggfunc='size', fill_value=0).reset_index()
    for c in ['입항','출항']:
        if c not in pivot_entryexit.columns: pivot_entryexit[c] = 0
    pivot_entryexit['총계'] = pivot_entryexit['입항'] + pivot_entryexit['출항']
    show_entryexit = pivot_entryexit.copy()
    for c in ['입항','출항','총계']:
        show_entryexit[c] = show_entryexit[c].map(lambda x: f"{int(x):,}")

    return (pivot_inout, show_inout), (pivot_entryexit, show_entryexit)
